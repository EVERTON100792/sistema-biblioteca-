...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-brand-surface text-slate-900 font-sans flex">
      {/* Sidebar */}
      <aside className="w-72 sidebar-gradient text-white flex flex-col shadow-2xl z-20">
        <div className="p-8">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        {/* Header */}
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200/50 flex items-center justify-between px-10 sticky top-0 z-10">
          <div>
            <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-0.5">Visão Geral</h2>
            <h1 className="text-2xl font-bold text-slate-900 capitalize">{activeTab}</h1>
          </div>
          <div className="flex items-center gap-6">
            <div className="relative group">
              <Search className="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-brand-accent transition-colors" />
              <input 
                type="text" 
                placeholder="Pesquisar registros..." 
                className="pl-11 pr-4 py-2.5 bg-slate-100 border-2 border-transparent rounded-xl text-sm focus:bg-white focus:border-brand-accent/20 outline-none w-72 transition-all"
                value={searchTerm}
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'books' && (
              ...
              >
                <div className="flex justify-between items-end">
                  <div>
                    <h3 className="text-2xl font-bold tracking-tight text-slate-900">Catálogo de Livros</h3>
                    <p className="text-sm text-slate-500 font-medium">Gerencie o acervo da biblioteca</p>
                  </div>
                  <button 
                    onClick={() => setShowBookForm(true)}
                    className="bg-brand-accent text-white px-6 py-3 rounded-2xl font-bold text-sm flex items-center gap-2 hover:bg-emerald-600 transition-all shadow-xl shadow-brand-accent/20 active:scale-95"
                  >
                    <Plus className="w-5 h-5" /> Adicionar Livro
                  </button>
                </div>

                <div className="bg-white rounded-[2rem] border border-slate-200/60 shadow-sm overflow-hidden flex flex-col max-h-[600px]">
                        ...
                        {filteredBooks.map(book => (
                          <tr key={book.id} className="hover:bg-slate-50/50 transition-colors group">
                            <td className="px-8 py-5">
                              <p className="font-bold text-sm text-slate-900 group-hover:text-brand-accent transition-colors">{book.title}</p>
                              <p className="text-xs text-slate-500 font-medium">{book.author}</p>
                            </td>
                            <td className="px-8 py-5">
                              <p className="text-xs font-bold font-mono text-slate-600">{book.isbn}</p>
                              <p className="text-xs text-slate-400 font-medium">{book.publisher}</p>
                            </td>
                            <td className="px-8 py-5">
                            ...
                            <td className="px-8 py-5 text-right">
                              <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                <button 
                                  ...
                                  className="p-2.5 bg-slate-100 text-slate-400 hover:text-brand-accent hover:bg-brand-accent/10 rounded-xl transition-all"
                                >
                                ...
                                <button 
                                  onClick={() => setConfirmDelete({ 
                                    type: 'book', 
                                    id: book.id, 
                                    message: `Tem certeza que deseja excluir o livro "${book.title}"?` 
                                  })}
                  ...
                  {filteredBooks.length === 0 && (
                    <div className="py-24 text-center">
                      ...
                      <p className="text-slate-400 font-medium">Nenhum livro encontrado no acervo.</p>
                    </div>
                  )}
                </div>
              </motion.div>
            ...
            {activeTab === 'students' && (
              ...
              >
                <div className="flex justify-between items-end">
                  <div>
                    <h3 className="text-2xl font-bold tracking-tight text-slate-900">Gestão de Alunos</h3>
                    <p className="text-sm text-slate-500 font-medium">Controle de membros da biblioteca</p>
                  </div>
                  <button 
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {filteredStudents.map(student => (
                          <tr key={student.id} className="hover:bg-slate-50/50 transition-colors group">
                            <td className="px-8 py-5">
                              <div className="flex items-center gap-4">
                                <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 font-bold text-xs group-hover:bg-brand-accent group-hover:text-white transition-colors">
            ...
            {activeTab === 'loans' && (
              ...
              >
                <div className="flex justify-between items-end">
                  <div>
                    <h3 className="text-2xl font-bold tracking-tight text-slate-900">Gestão de Empréstimos</h3>
                    <p className="text-sm text-slate-500 font-medium">Acompanhamento de retiradas e devoluções</p>
                  </div>
                  <button 
                ...
                <div className="bg-white rounded-[2rem] border border-slate-200/60 shadow-sm overflow-hidden flex flex-col max-h-[600px]">
                          ...
                          <tr key={loan.id} className="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td className="px-8 py-5">
                              <div className="flex items-center gap-3">
                                ...
                                <p className="text-sm font-bold text-slate-700">{loan.bookTitle}</p>
                              </div>
                            </td>
                            <td className="px-8 py-5">
                              <div className="flex flex-col gap-1.5">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-[10px] uppercase font-bold text-slate-400">Saída: {format(parseISO(loan.loanDate), 'dd/MM/yyyy')}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <span className="w-1 h-1 bg-brand-accent rounded-full" />
                                  <span className="text-[10px] uppercase font-bold text-brand-accent">Prazo: {format(parseISO(loan.dueDate), 'dd/MM/yyyy')}</span>
                                </div>
                              </div>
                            </td>
                            <td className="px-8 py-5">
                              ...
                              <div className="flex items-center justify-end gap-2">
                                {!loan.returnDate && (
                                  <button 
                                    ...
                                    className="text-[10px] font-bold uppercase tracking-widest text-white bg-brand-accent hover:bg-emerald-600 px-4 py-2 rounded-xl transition-all shadow-lg shadow-brand-accent/10"
                                  >
                                    Dar Baixa
                                  </button>
                                )}
                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                  <button 
                                    onClick={() => { setEditingLoan(loan); setShowLoanForm(true); }}
                                    className="p-2.5 bg-slate-100 text-slate-400 hover:text-brand-accent hover:bg-brand-accent/10 rounded-xl transition-all"
                                  >
                                    <Edit2 className="w-4 h-4" />
                                  </button>
                  ...
                  {filteredLoans.length === 0 && (
                    <div className="py-24 text-center">
                      ...
                      <p className="text-slate-400 font-medium">Nenhum registro de empréstimo.</p>
                    </div>
                  )}
                </div>
              </motion.div>
